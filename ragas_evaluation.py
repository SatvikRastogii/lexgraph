...
# Sentinel distinguishing "use the default file path" from "explicitly skip
# writing this output" (passing None), since None is also a valid file-less choice.
_UNSET = object()


def run_evaluation(questions=None, results_file=_UNSET, report_file=_UNSET):
    """Run RAGAS evaluation: Naive RAG vs GraphRAG.

    questions: optional {category: [question, ...]} dict. When omitted, loads
        the full benchmark_questions.json (original/default behavior).
    results_file / report_file: optional output path overrides. Pass None to
        skip writing that output entirely (used by the lightweight drift-check
        job, which doesn't need a markdown report). Omit to use the module
        defaults (RESULTS_FILE / REPORT_FILE).

    Returns the list of per-question result entries.
    """
    results_file = RESULTS_FILE if results_file is _UNSET else results_file
    report_file = REPORT_FILE if report_file is _UNSET else report_file
    ...
    # ── Generate report ──
    if report_file:
        generate_report(results, report_file=report_file)

    total_time = round((time.perf_counter() - eval_start) / 60, 1)
    print(f"\n{'=' * 70}")
    print(f"  ✅ RAGAS Evaluation Complete! ({total_time} minutes)")
    print(f"  📄 Raw data:  {results_file or '(skipped)'}")
    print(f"  📊 Report:    {report_file or '(skipped)'}")
    print(f"{'=' * 70}")
...
def generate_report(results, report_file=REPORT_FILE):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LexGraph RAGAS evaluation engine")
    parser.add_argument("--sample-size", type=int, default=None,
                         help="Evaluate only the first N questions per category (default: all)")
    parser.add_argument("--results-file", default=None, help="Override path for raw JSON results")
    parser.add_argument("--report-file", default=None, help="Override path for the markdown report")
    ...
    if args.sample_size is not None:
        ...
        questions_subset = {cat: qs[: args.sample_size] for cat, qs in all_questions.items()}

    run_evaluation(
        questions=questions_subset,
        results_file=args.results_file if args.results_file is not None else _UNSET,
        report_file=args.report_file if args.report_file is not None else _UNSET,
    )
